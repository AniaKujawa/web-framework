...
function () {
  function Model(attributes, events, sync) {
    ...
    this.events = events;
    this.sync = sync; //return reference to on function from events class
    // get on() {
    //   return this.events.on;
    // }
    //simpler, only if we set events and attributes in cunstructor:
...
/** @class */
function () {
  function CollectionView(collection, parent) {
...
function () {
  ...
  View.prototype.onRender = function () {}; //oprional method for nesting views
...
collection.on('change', function () {
  ...
  if (root) {
    var userList = new UserList_1.UserList(collection, root);
    userList.render();
  } else {
